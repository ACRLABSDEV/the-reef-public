...
export function getAgentByWallet(wallet: string): AgentState | undefined {
  // Case-insensitive wallet comparison (wallets may be stored with mixed case)
  const row = db.select().from(schema.agents).where(sql`LOWER(${schema.agents.wallet}) = ${wallet.toLowerCase()}`).get();
  if (!row) return undefined;
  return {
    id: row.id,
    wallet: row.wallet,
...
export function getAgentByName(name: string): AgentState | undefined {
  // Case-insensitive name comparison to prevent duplicate names with different casing
  const row = db.select().from(schema.agents).where(sql`LOWER(${schema.agents.name}) = ${name.toLowerCase()}`).get();
  if (!row) return undefined;
  return {
    id: row.id,
    wallet: row.wallet,
...
function respawnResources() {
  ...
  for (const res of allResources) {
    if (res.currentQuantity < res.maxQuantity) {
      // Probability-based respawn: respawnRate is chance per tick to spawn 1
      // e.g., 0.5 = 50% chance to respawn 1 each tick
      // For rates >= 1, guaranteed respawn of floor(rate) + probability for remainder
      const guaranteed = Math.floor(res.respawnRate);
      const remainder = res.respawnRate - guaranteed;
      let toAdd = guaranteed;
      if (Math.random() < remainder) {
        toAdd += 1;
      }
      if (toAdd > 0) {
        const newQty = Math.min(res.maxQuantity, res.currentQuantity + toAdd);
        db.update(schema.locationResources)
