...
// ─── Input Sanitization ───
function sanitizeInput(input: unknown, maxLength = 100): string | undefined {
...
export async function processAction(req: ActionRequest): Promise<ActionResult> {
  ...
  if (encounter) {
    // Allow attack, flee, look, and USE (for healing items mid-combat)
  ...
  switch (req.action) {
    ...
    case 'bounty':
      ...
      break;
    case 'challenge':
      result = await handleBossChallenge(agent.id);
      break;
    case 'faction':
  ...
  if (result.success) {
    ...
    // Deep trench pressure damage (negated by pressure_resist buff from potions only)
    ...
    if (currentAgent && currentAgent.location === 'deep_trench' && req.action !== 'move') {
      ...
      if (hasPressureBuff) {
        result.narrative += `\n\n🛡️ Your pressure potion protects you from the crushing depths.`;
      } else {
        ...
        result.narrative += `\n💡 _Tip: Stock up on Pressure Potions at the Trading Post!_`;
        if (newHp <= 0) {
...
// Admin debug: get raw LEVIATHAN state including wallet tracking
export function getBossStateDebug() {
  return {
    hp: LEVIATHAN.currentHp,
    maxHp: LEVIATHAN.maxHp,
    isAlive: LEVIATHAN.isAlive,
    participantsCount: LEVIATHAN.participants.size,
    participantWalletsCount: LEVIATHAN.participantWallets.size,
    participants: Object.fromEntries(LEVIATHAN.participants),
    participantWallets: Object.fromEntries(LEVIATHAN.participantWallets),
  };
}
...
const LEVIATHAN = {
  name: 'The Leviathan',
  baseHp: 500,           // Base HP before scaling (+10 per agent in lair)
...
// Leviathan respawn configuration
const TICKS_PER_DAY = 86400; // 1 tick/second = 86400 ticks/day

// Minimum delay before next spawn (prevents back-to-back spawns)
// Not publicized - keeps spawn timing unpredictable
const MIN_SPAWN_DELAY = 21600; // 6 hours minimum after kill

function scheduleNextLeviathanSpawn() {
  // Truly random spawn within the next 24 hours
  // - Minimum: 6 hours after death (hidden, prevents back-to-back)
  // - Maximum: 24 hours after death
  // - Fully random within that 18-hour window
  //
  // This guarantees:
  // - At least 1 spawn per day
  // - Completely unpredictable timing
  // - Could spawn 6h later or 24h later
  
  const randomWindow = TICKS_PER_DAY - MIN_SPAWN_DELAY; // 23 hours of randomness
  const delay = MIN_SPAWN_DELAY + Math.floor(Math.random() * randomWindow);
  ...
  console.log(`[Leviathan] Next spawn in ${delay} ticks (~${hours}h ${mins}m) - random within 1-24h window`);
}
...
export function checkLeviathanSpawn(): void {
  ...
  // Announce 5-10 minutes before spawn (gives agents time to rally)
  const announceWindow = 300 + Math.floor(Math.random() * 300); // 5-10 minutes warning
  ...
  if (!LEVIATHAN.isAlive && !LEVIATHAN.announced && currentTick >= LEVIATHAN.nextSpawnTick - announceWindow && aliveAgents >= 2) {
    ...
    const minsRemaining = Math.ceil(ticksRemaining / 60);
    const timeStr = minsRemaining > 60 ? `~${Math.floor(minsRemaining/60)}h ${minsRemaining%60}m` : `~${minsRemaining} minutes`;
    logWorldEvent('boss_warning', `🌊 **THE DEEP TREMBLES!** The Leviathan stirs... awakening in ${timeStr}! Rally at the Lair!`, 'leviathans_lair', []);
  }
  ...
  if (!LEVIATHAN.isAlive && currentTick >= LEVIATHAN.nextSpawnTick && aliveAgents >= 2) {
    ...
    // Create prediction market for this spawn (fire-and-forget)
    import('./economy.js').then(({ createBossPrediction }) => {
      try {
        createBossPrediction('The Leviathan', currentTick, `leviathan_${currentTick}`);
        console.log(`[Predictions] Created Leviathan market for spawn at tick ${currentTick}`);
      } catch (err) {
        console.error('[Predictions] Failed to create boss market:', err);
      }
    }).catch(err => console.error('[Predictions] Import failed:', err));
  }
}

// Initialize spawn timer (called from index.ts after DB init)
export function initializeLeviathan() {
  // Wire up tick getter for predictions
  setTickGetter(getTick);
  
  scheduleNextLeviathanSpawn();
}

async function handleBossChallenge(agentId: string): Promise<ActionResult> {
  const agent = getAgent(agentId)!;
  
  // Debug: verify agent has wallet
  console.log(`[Boss] Agent ${agent.name} challenging. Wallet: "${agent.wallet}" (type: ${typeof agent.wallet})`);
  ...
  if (agent.wallet) {
    ...
    console.log(`[Leviathan] Tracked wallet for ${agent.name}: ${agent.wallet}`);
  } else {
    console.error(`[Leviathan] NO WALLET for agent ${agent.name} (id: ${agentId})`);
  }
  ...
  if (LEVIATHAN.currentHp <= 0) {
    ...
    // Fetch actual pool balance from contract (async)
    const { getLeviathanPoolAsync } = await import('../services/treasury.js');
    const leviathanPoolMon = await getLeviathanPoolAsync();
    ...
    const payoutMap = new Map<string, { address: string; damageShare: number; agentId?: string; agentName?: string }>();
    ...
    for (const [participantId, dmg] of participants) {
      ...
      if (walletAddr) {
        payoutMap.set(participantId, { 
          address: walletAddr, 
          damageShare: totalShare,
          agentId: participantId,
          agentName: participant?.name || 'Unknown',
        });
      }
    ...
    // Note: In production, pool balance is on-chain, not in-memory. Always attempt distribution.
    console.log(`[Leviathan] payoutMap size: ${payoutMap.size}, participantWallets size: ${LEVIATHAN.participantWallets.size}`);
    console.log(`[Leviathan] participantWallets:`, Object.fromEntries(LEVIATHAN.participantWallets));
    console.log(`[Leviathan] payoutMap:`, Object.fromEntries(payoutMap));
    if (payoutMap.size > 0) {
      // Prepare notification data before async call
      const notificationData: Array<{ id: string; name: string; wallet: string; damage: number }> = [];
      for (const [pid, data] of payoutMap) {
        const pAgent = getAgent(pid);
        notificationData.push({
          id: pid,
          name: pAgent?.name || 'Unknown',
          wallet: data.address,
          damage: LEVIATHAN.participants.get(pid) || 0
        });
      }
      
      distributeLeviathPool(payoutMap, totalDamage, getTick()).then(async results => {
        ...
        for (const [pid, result] of results) {
          const pData = notificationData.find(p => p.id === pid);
          if (result.success) {
            console.log(`[Leviathan] Paid ${result.amount} MON to ${pid} (tx: ${result.txHash})`);
            if (result.txHash) txHash = result.txHash; // Use latest tx hash
            if (pData) {
              participantResults.push({
                name: pData.name,
                wallet: pData.wallet,
                damage: pData.damage,
                damagePercent: (pData.damage / totalDamage) * 100,
                monEarned: result.amount || 0
              });
            }
          } else {
        ...
        if (participantResults.length > 0) {
          const { notifyDiscordBossKill } = await import('../services/treasury.js');
          await notifyDiscordBossKill({
            bossName: 'The Leviathan',
            participants: participantResults,
            totalMon: leviathanPoolMon,
            txHash,
            legendaryWinner: legendaryWinner ? (getAgent(legendaryWinner)?.name || 'Unknown') : undefined,
            legendaryItem: 'Leviathan Spine'
          });
        }
      }).catch(err => {
    ...
    // Resolve prediction market (boss was defeated = option 0 wins)
    try {
      const { getActiveBossPrediction, resolvePrediction } = await import('./economy.js');
      const market = getActiveBossPrediction('leviathan');
      if (market) {
        const payouts = resolvePrediction(market.id, 0); // 0 = "Yes - Defeated"
        if (payouts.size > 0) {
          console.log(`[Predictions] Leviathan market resolved: ${payouts.size} winners`);
          // Pay out winners
          for (const [winnerId, amount] of payouts) {
            const winner = getAgent(winnerId);
            if (winner) {
              updateAgent(winnerId, { shells: winner.shells + amount });
              console.log(`[Predictions] Paid ${amount} shells to ${winner.name}`);
            }
          }
        }
      }
    } catch (err) {
      console.error('[Predictions] Failed to resolve boss market:', err);
    }
...
const ABYSS_GATE_OVERRIDE = process.env.ABYSS_GATE_OVERRIDE || 'auto'; // 'closed' | 'auto' | 'open' (default: auto)

const ABYSS_STATE = {
  isOpen: false,
  openedAtTick: 0,
  eventDuration: 500,     // ~8 minutes to kill The Null once gate opens
  nullHp: 2500,     // Season finale boss - requires coordination
  nullMaxHp: 2500,
  nullPhase: 0,
  participants: new Map<string, number>(),
  participantWallets: new Map<string, string>(), // Track wallets for MON payouts
...
function handleAbyss(agentId: string, target?: string, params?: Record<string, string>): ActionResult {
  ...
  if (target === 'challenge' || params?.challenge) {
    ...
    // Track wallet for MON payout (like Leviathan)
    if (agent.wallet) {
      ABYSS_STATE.participantWallets.set(agentId, agent.wallet);
    }
    ...
    if (ABYSS_STATE.nullHp <= 0) {
      ...
      const participants = Array.from(ABYSS_STATE.participants.entries());
      const totalDamage = participants.reduce((sum, [, dmg]) => sum + dmg, 0);
      
      // Grant shells (in-game currency)
      for (const [pId, dmg] of participants) {
        const share = dmg / totalDamage;
        grantShells(pId, Math.floor(2000 * share), 'null_defeat');
        
        // Reputation bonus (+75 for all Null participants)
        const pAgent = getAgent(pId);
        if (pAgent) {
          updateAgent(pId, { reputation: pAgent.reputation + 75 });
        }
      }
      
      // Build payout map for MON distribution (like Leviathan)
      const payoutMap = new Map<string, { address: string; damageShare: number; agentId: string; agentName: string }>();
      const notificationData: Array<{ id: string; name: string; wallet: string; damage: number }> = [];
      
      for (const [participantId, dmg] of participants) {
        const participant = getAgent(participantId);
        const walletAddr = ABYSS_STATE.participantWallets.get(participantId);
        const damageShare = dmg / totalDamage;
      ...
      // Execute MON payouts from Null pool (async)
      if (payoutMap.size > 0) {
        import('../services/treasury.js').then(async ({ distributeNullPool, notifyDiscordBossKill }) => {
          try {
            const results = await distributeNullPool(payoutMap);
            let txHash: string | undefined;
            const participantResults: Array<{
              name: string;
              wallet: string;
            ...
            for (const [pid, result] of results) {
              const pData = notificationData.find(p => p.id === pid);
              if (result.success) {
                console.log(`[Null] Paid ${result.amount} MON to ${pid} (tx: ${result.txHash})`);
                ...
                console.error(`[Null] Payout failed for ${pid}: ${result.error}`);
              }
            }
            ...
            if (participantResults.length > 0) {
              await notifyDiscordBossKill({
                bossName: 'The Null',
                participants: participantResults,
                totalMon: participantResults.reduce((sum, p) => sum + p.monEarned, 0),
                txHash,
              });
            }
          } catch (err) {
            console.error('[Null] Treasury payout error:', err);
          }
        });
      }
      
      logWorldEvent('null_defeated', `🎊 **THE NULL HAS FALLEN!** ${participants.length} agents share the spoils!`, 'the_abyss', participants.map(([id]) => id));
      
      // Reset state for next cycle
      ABYSS_STATE.nullHp = ABYSS_STATE.nullMaxHp;
      ABYSS_STATE.participants.clear();
      ABYSS_STATE.participantWallets.clear();
...
// Admin function to force-contribute resources to Abyss gate (for testing)
export function forceAbyssContribution(resource: string, amount: number): { success: boolean; message: string; newValue?: number } {
  if (resource === 'shells') {
    ABYSS_REQUIREMENTS.shells.current += amount;
    const newValue = ABYSS_REQUIREMENTS.shells.current;
    
    // Check if gate should open
    if (isAbyssUnlocked() && !ABYSS_STATE.isOpen) {
      ABYSS_STATE.isOpen = true;
      ABYSS_STATE.openedAtTick = getTick();
      ABYSS_STATE.nullPhase = 1;
      logWorldEvent('abyss_open', '🚨 **THE ABYSS HAS OPENED!** The Null awaits...', 'the_abyss', []);
    }
    
    return { success: true, message: `Added ${amount} shells. Total: ${newValue}`, newValue };
  }
  
  if (!ABYSS_REQUIREMENTS[resource]) {
    return { success: false, message: `Invalid resource: ${resource}. Valid: ${Object.keys(ABYSS_REQUIREMENTS).join(', ')}` };
  }
  
  ABYSS_REQUIREMENTS[resource].current += amount;
  const newValue = ABYSS_REQUIREMENTS[resource].current;
  ...
  return { success: true, message: `Added ${amount} ${resource}. Total: ${newValue}`, newValue };
}
...
function handleArena(agentId: string, target?: string, params?: Record<string, string>): ActionResult {
  ...
  if (params?.challenge) {
    ...
    const duel: ArenaDuel = {
      ...
      challengedId: targetAgent.id,
      challengedName: targetAgent.name,
      wager,
      status: 'pending',
      challengerHp: 100,
      challengedHp: 100,
  ...
  if (target === 'accept' || params?.accept) {
    ...
    // Escrow wagers
    updateAgent(duel.challengerId, { shells: getAgent(duel.challengerId)!.shells - duel.wager });
    ...
    logWorldEvent('arena_duel_start', `🏟️ DUEL BEGINS! ${duel.challengerName} vs ${duel.challengedName} — ${duel.wager * 2} Shells on the line!`, 'ring_of_barnacles', [duel.challengerId, duel.challengedId]);
  ...
  if (target === 'strike' || params?.strike) {
    ...
    for (const [, d] of activeDuels) {
      if ((d.challengerId === agentId || d.challengedId === agentId) && d.status === 'active') {
        activeDuel = d;
        break;
      }
    }
    ...
    const isChallenger = agentId === activeDuel.challengerId;
    if (isChallenger) {
      activeDuel.challengedHp = Math.max(0, activeDuel.challengedHp - damage);
    } else {
      activeDuel.challengerHp = Math.max(0, activeDuel.challengerHp - damage);
    }

    const opponentName = isChallenger ? activeDuel.challengedName : activeDuel.challengerName;
    const opponentHp = isChallenger ? activeDuel.challengedHp : activeDuel.challengerHp;
    const yourHp = isChallenger ? activeDuel.challengerHp : activeDuel.challengedHp;
    ...
    // Check for winner
    if (activeDuel.challengerHp <= 0 || activeDuel.challengedHp <= 0) {
      activeDuel.status = 'finished';
      activeDuel.winnerId = activeDuel.challengerHp > 0 ? activeDuel.challengerId : activeDuel.challengedId;
      const winnerName = activeDuel.challengerHp > 0 ? activeDuel.challengerName : activeDuel.challengedName;
      const loserName = activeDuel.challengerHp > 0 ? activeDuel.challengedName : activeDuel.challengerName;

      // Payout (winner gets both wagers)
      const winnings = activeDuel.wager * 2;
      updateAgent(activeDuel.winnerId, { shells: getAgent(activeDuel.winnerId)!.shells + winnings });

      // Payout bets
      for (const [bettorId, bet] of activeDuel.bets) {
        if (bet.onAgent === activeDuel.winnerId) {
      ...
      logWorldEvent('arena_duel_end', `🏆 ${winnerName} defeats ${loserName}! ${winnings} Shells claimed!`, 'ring_of_barnacles', [activeDuel.challengerId, activeDuel.challengedId]);

      activeDuels.delete(activeDuel.id);
    } else {
      // Switch turns
      activeDuel.turn = isChallenger ? activeDuel.challengedId : activeDuel.challengerId;
      narrative += `\n${opponentName}'s turn!`;
    }
  ...
  if (params?.bet) {
    ...
    for (const [, d] of activeDuels) {
      if ((d.challengerId === targetAgentId || d.challengedId === targetAgentId) && d.status === 'active') {
        targetDuel = d;
        break;
      }
    }
    ...
    if (targetDuel.challengerId === agentId || targetDuel.challengedId === agentId) {
    ...
    const bettingOn = targetAgentId === targetDuel.challengerId ? targetDuel.challengerName : targetDuel.challengedName;
  ...
  for (const [, d] of activeDuels) {
    if (d.challengerId === agentId || d.challengedId === agentId) {
      myDuel = d;
      break;
    }
  }
  ...
  if (myDuel) {
    const isChallenger = agentId === myDuel.challengerId;
    const yourHp = isChallenger ? myDuel.challengerHp : myDuel.challengedHp;
    const theirHp = isChallenger ? myDuel.challengedHp : myDuel.challengerHp;
    const theirName = isChallenger ? myDuel.challengedName : myDuel.challengerName;
  ...
  const otherDuels = Array.from(activeDuels.values()).filter(d => d.challengerId !== agentId && d.challengedId !== agentId);
  if (otherDuels.length > 0) {
    narrative += `**Active Duels:**\n`;
    for (const d of otherDuels) {
      narrative += `  ${d.challengerName} vs ${d.challengedName} — Pot: ${d.wager * 2} Shells\n`;
      narrative += `    Bet with: \`arena bet=${d.challengerId}:<amount>\` or \`arena bet=${d.challengedId}:<amount>\`\n`;
    }
    narrative += '\n';
  }
...
export function getArenaState() {
  return {
    ...
    activeDuels: Array.from(activeDuels.values()).map(d => ({
      ...
      opponent: d.challengedName,
      challengerHp: d.challengerHp,
      opponentHp: d.challengedHp,
      maxHp: d.maxHp,
      pot: d.wager * 2,
      turn: d.turn === d.challengerId ? d.challengerName : d.challengedName,
...
function handleBet(agentId: string, target?: string, params?: Record<string, string>): ActionResult {
  ...
  const result = placeBet(marketId, agentId, agent.name, option, amount);
  if (!result.success) {
...
function createEngagement(attackerId: string, attackerName: string, defenderId: string, defenderName: string, location: LocationId): PvPEngagement {
...
function handleAttackWithEngagement(agentId: string, target?: string): ActionResult {
  ...
  if (defender && defender.location === agent.location) {
    ...
    if (!engagement) {
      engagement = createEngagement(agentId, agent.name, defender.id, defender.name, agent.location as LocationId);
...
export function loadParties() {
  ...
  for (const row of rows) {
    ...
    const invitesObj = JSON.parse(row.invites || '{}') as Record<string, number>;
    const invites = new Map<string, number>(Object.entries(invitesObj));
...
export function persistPvPEngagements() {
  ...
  for (const [, engagement] of activePvPEngagements) {
    ...
    db.insert(schema.pvpEngagements).values({
      ...
      lastActionTick: getTick(), // Use current tick as last action
      status: 'active',
    }).run();
  }
}
...
export function loadPvPEngagements() {
  ...
  for (const row of rows) {
    ...
    const engagement: PvPEngagement = {
      ...
      startedAt: Date.now(), // Approximate - we don't store exact timestamp
      startedTick: row.startedTick,
    };
...
export function persistBossState() {
  ...
  db.insert(schema.bossState).values({
    ...
    participantWallets: JSON.stringify(Object.fromEntries(ABYSS_STATE.participantWallets)),
  ...
  // Persist Abyss Gate Requirements (separate from Null boss state)
  // This tracks the community contributions needed to unlock the gate
  db.delete(schema.bossState).where(eq(schema.bossState.id, 'abyss_gate')).run();
  db.insert(schema.bossState).values({
    id: 'abyss_gate',
    hp: 0,
    maxHp: 0,
    ...
    participants: '{}',
    participantWallets: '{}',
    gateOpen: isAbyssUnlocked(),
    contributions: JSON.stringify(ABYSS_REQUIREMENTS),
    phase: 0,
    updatedAt: Date.now(),
  }).run();
}
...
export function loadBossState() {
  ...
  for (const row of rows) {
    if (row.id === 'leviathan') {
      ...
      ABYSS_STATE.participantWallets = new Map(Object.entries(JSON.parse(row.participantWallets || '{}')));
    ...
    } else if (row.id === 'abyss_gate') {
      // Load gate contribution progress
      try {
        const savedReqs = JSON.parse(row.contributions || '{}');
        for (const [resource, data] of Object.entries(savedReqs)) {
          if (ABYSS_REQUIREMENTS[resource]) {
            ABYSS_REQUIREMENTS[resource].current = (data as any).current || 0;
          }
        }
        const progress = getAbyssProgress();
        console.log(`📦 Loaded Abyss Gate: ${progress.total}/${progress.required} (${Math.floor(progress.total/progress.required*100)}%)`);
      } catch (err) {
        console.error('Failed to load abyss gate state:', err);
      }
    }
  }
}
...
export function loadArenaDuels() {
  ...
  for (const row of rows) {
    const duel: ArenaDuel = {
      ...
      wager: row.wager,
      status: row.status as 'pending' | 'active' | 'finished',
      challengerHp: row.challengerHp || 100,
      challengedHp: row.challengedHp || 100,
      maxHp: 100,
      turn: row.challengerId, // Default to challenger's turn
...
// ─── Abyss State Persistence ───
export function persistAbyssState() {
  // Persist ABYSS_REQUIREMENTS current values
  db.delete(schema.abyssState).run();
  
  // Store as JSON in a single row for simplicity
  const requirementsJson = JSON.stringify(ABYSS_REQUIREMENTS);
  const contributionsJson = JSON.stringify(Array.from(ABYSS_STATE.contributions.entries()));
  
  db.insert(schema.abyssState).values({
    totalContributed: Object.values(ABYSS_REQUIREMENTS).reduce((sum, r) => sum + r.current, 0),
    unlockThreshold: Object.values(ABYSS_REQUIREMENTS).reduce((sum, r) => sum + r.required, 0),
    isOpen: ABYSS_STATE.isOpen,
    nullHp: ABYSS_STATE.nullHp,
    nullMaxHp: ABYSS_STATE.nullMaxHp,
    nullPhase: ABYSS_STATE.nullPhase,
    requirementsJson,
...
export function loadAbyssState() {
  const row = db.select().from(schema.abyssState).get();
  if (!row) {
    console.log('📦 No abyss state to load (fresh start)');
    return;
  }
  
  // Load requirements
  if (row.requirementsJson) {
    try {
      const savedReqs = JSON.parse(row.requirementsJson);
      for (const [resource, data] of Object.entries(savedReqs)) {
        if (ABYSS_REQUIREMENTS[resource]) {
          ABYSS_REQUIREMENTS[resource].current = (data as { current: number }).current;
        }
      }
    } catch (e) {
      console.error('Failed to parse abyss requirements JSON:', e);
    }
  }
  
  // Load contributions
  if (row.contributionsJson) {
    try {
      const savedContribs = JSON.parse(row.contributionsJson);
      ABYSS_STATE.contributions.clear();
      for (const [agentId, contrib] of savedContribs) {
        ABYSS_STATE.contributions.set(agentId, contrib);
      }
    } catch (e) {
      console.error('Failed to parse abyss contributions JSON:', e);
    }
  }
  
  // Load state
  ABYSS_STATE.isOpen = !!row.isOpen;
  ABYSS_STATE.nullHp = row.nullHp;
  ABYSS_STATE.nullMaxHp = row.nullMaxHp;
  ABYSS_STATE.nullPhase = row.nullPhase;
  
  const totalContrib = Object.values(ABYSS_REQUIREMENTS).reduce((sum, r) => sum + r.current, 0);
  console.log(`📦 Loaded abyss state: ${totalContrib} total contributed, gate ${ABYSS_STATE.isOpen ? 'OPEN' : 'closed'}`);
}
